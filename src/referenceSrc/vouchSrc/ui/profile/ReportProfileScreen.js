...
import ReportProfileService from '../../services/ReportProfileService';
const { height, width } = Dimensions.get("screen");

class ReportProfileScreen extends Component {
  ...
  reportProfileApi = (userId, message) => {
    if (message.length >= 3) {
      this.setState({ isVisible: true })
      new ReportProfileService(userId, message).reportProfile().then(response => {
        if (response) {
    ...
    return (
      <View style={styles.submitButton}>
        <AppButton
          ...
          borderColor={"#ff9c00"}
          textColor={"white"}
          onPress={() => this.reportProfileApi(this.state.userId, this.state.message)}
        />
      </View>
    )
  }
