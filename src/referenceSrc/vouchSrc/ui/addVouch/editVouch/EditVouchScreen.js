...
class EditVouchScreen extends Component {
  ...
  //ComponentDidMount
  componentDidMount() {
    console.log('selected vouch details is = ',this.props.route?.params?.vouchData)
    // console.log('selected suggested details is = ',this.props.route?.params?.suggested)
    //header method is used for rendering custimize header
    this.header();
    //viewshotMethod used for taking screen shot
    this.viewShotMethod();
  ...
  updateVouchApi = (feed_id) => {
    ...
    const params = {
      ...
      // type : type,
      http_link: httpLink,
      suggestedBy:this.props?.route?.params?.userId ? this.props?.route?.params?.userId : this.props?.route?.params?.firstName ? this.props?.route?.params?.firstName:"",
      userEmail:this.props?.route?.params?.notExistingUserEmail ? this.props?.route?.params?.notExistingUserEmail : "",
    }
  ...
  addVouchApi = () => {
    ...
    formData.append("privacyStatus", privacyStatus);   
    formData.append("categoryId", categoryId); 
    formData.append("suggestedBy", this.props?.route?.params?.userId ? this.props?.route?.params?.userId : this.props?.route?.params?.firstName ? this.props?.route?.params?.firstName:"");
    formData.append("userEmail", this.props?.route?.params?.notExistingUserEmail ? this.props?.route?.params?.notExistingUserEmail : "");
    formData.append("isTry", isTry);
    formData.append("type", type);
    formData.append("httpLink", httpLink);
  ...
  onSave = () => {
    ...
    AsyncStorage.getItem("isChecked").then((value) => {
      ...
      if (value == 'Checked') {
        ...
        this.setState({ isDefaultModal: true })
        this.setState({ isChecked: false });
      }
    })
  }
  ...
  whoVouchedForUser = () => {
    console.log("All non exist use details is = ",this.props?.route?.params)
