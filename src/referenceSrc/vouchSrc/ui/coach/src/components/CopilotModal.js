...
import { Animated, Easing, View, NativeModules, Modal, StatusBar,TouchableOpacity, Platform,Text, SafeAreaView } from 'react-native';
...
class CopilotModal extends Component<Props, State> {
  ...
  async _animateMove(obj = {}): void {
    ...
    // console.log("relativeToLeft > relativeToRight",relativeToLeft,relativeToRight);
    // console.log("relativeToBottom > relativeToTop",relativeToBottom , relativeToTop)
    const tooltip = {};
    const arrow = {};
    ...
    } else if(relativeToLeft + 10 > relativeToRight){
      tooltip.right = Math.max(layout.width - (obj.left + obj.width), 0);
      tooltip.right = tooltip.right === 0 ? tooltip.right + MARGIN : tooltip.right;
      tooltip.maxWidth = layout.width - tooltip.right - MARGIN;
      arrow.right = tooltip.right + MARGIN;
  ...
  renderTooltip() {
    ...
    return [
      ...
      >
        ...
        >
          <Text style={{ color: "white",fontSize:18,fontFamily:fonts.SanFrancisco.Bold }}>End Tour</Text>
        </TouchableOpacity>
      </Animated.View>,
      // <Animated.View
      //   key="stepNumber"
