...
import {
  ...
  Image,
  ActivityIndicator,
  Keyboard, DeviceEventEmitter, Platform
} from "react-native";
...
const FeedDetailScreen = (props) => {
  ...
  const renderItem = ({ item }) => <Comment {...props} item={item} ownVouch={vouchData.ownVouch} loginUserID={userData.userId}/>;
  var commentNeedToFetch = false
  var main_vouchID = undefined
  React.useEffect(() => {
    header()
    getUserDetail();
  }, []);
  ...
  const getFeedDetail = (feedId) => {
    new FeedDetailService(feedId)
      ...
      .then((response) => {
        ...
        if (commentNeedToFetch){
          commentNeedToFetch = false
          main_vouchID = response.vouch.id
          getCommentList(response.vouch.id, 1);
        }
      })
  ...
  const fetchMoreComments = (vouchId, pageNum, lastPage) => {
    console.log("fetchMoreComments id", vouchId, pageNum + 1);
  ...
  function refresh_feed(action) {
    checkFeedDetails()
    if (action){
      if (action === "vouched"){
        vouchData.isTry
        ? suggestionDeleteApi(vouchData.id, true)
  ...
  return vouchData && Object.keys(vouchData).length !== 0 ? (
    ...
    >
      <FlatList
        ...
        onEndReached={() => {
          fetchMoreComments(
            vouchData.id ? vouchData.id : props?.route?.params?.data.id,
            currentPage,
            lastPage
          );
        }}
